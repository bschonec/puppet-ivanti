  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      context 'Checking for package installation' do
        ...
        packages = %w[ivanti-software-distribution ivanti-base-agent ivanti-pds2 ivanti-schedule ivanti-inventory ivanti-vulnerability ivanti-cba8]
      ...
      config_files = %w[agent_settings broker_config inventory policy schedule software_distribution vulnerability hardware]
      ...
      execs = %w[agent_settings broker_config ldiscan vulscan map-fetchpolicy]
      context 'Check Exec resources' do
        execs.each do |exec|
          it {
            is_expected.to contain_exec(exec).
              with_command("/opt/landesk/bin/#{exec} -V").
              with_user('root').
